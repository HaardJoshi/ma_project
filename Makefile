...
combine:  ## Combine raw CSV exports → data/interim/ma_combined.csv
	$(PYTHON) scripts/data/run_combine.py

clean-data:  ## Clean combined CSV → data/interim/ma_cleaned.csv
	$(PYTHON) scripts/data/run_cleaning.py

preprocess:  ## Winsorise, z-score, split → data/processed/
	$(PYTHON) scripts/data/run_preprocessing.py --config $(CONFIG)

edgar-fetch:  ## Fetch 10-K filings from SEC EDGAR (set EMAIL=)
	$(PYTHON) scripts/features/run_edgar_fetch.py --email "$(EMAIL)"

train:  ## Train a model (use CONFIG=configs/xxx.yaml)
	$(PYTHON) scripts/training/train.py --config $(CONFIG)

evaluate:  ## Evaluate a trained model
	$(PYTHON) scripts/training/evaluate.py --config $(CONFIG)
