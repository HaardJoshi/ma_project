...
//  Appendix B: Evaluation of Product and Process
...
*Hyperparameter search was counterproductive.* The Bayesian hyperparameter search via Optuna was invested with disproportionate engineering effort relative to its eventual contribution. It reduced AUC across every configuration. In retrospect, for this type of high-noise financial prediction problem, a simple grid search over a small number of regularisation parameters would have been cheaper, faster, and more interpretable. The lesson is domain-specific: in noisy financial signal environments, architectural diversity (what features you include) is more valuable than optimiser sophistication (how finely you tune the same feature set). Future work should invest optimiser effort only after establishing that the feature space contains enough signal to reward fine-grained search.
