// ============================================================
//  04-findings.typ  (v3 — Reader-First, Methodology-Aligned)
...
All reported classification results derive from five-fold stratified cross-validation. All preprocessing steps — median imputation, scaling, and any trainable transformations — were fit on training folds only, then applied to the held-out fold, preserving the leakage controls established in Chapter 3 @mackinlay1997 @creswell2014.
...
The result is economically reassuring. Traditional financial features still dominate the very top of the ranking, which is exactly what the corporate finance literature would predict. But graph embedding components and PCA-compressed text components also appear among the strongest contributors, and their SHAP contributions are not uniformly close to zero. That pattern matters because it shows the multimodal lift is not a cross-validation accident. The model is extracting real, non-trivial signal from graph and text features that linear tabular models would either ignore or fail to combine properly @lundberg2017 @palepu1986.
