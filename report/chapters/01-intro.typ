...
The central aim of this study is to construct, evaluate, and interpret a heterogeneous multimodal machine learning framework for M&A synergy prediction that transcends traditional tabular limitations by integrating financial, textual, and topological data. The evolution of these aims from the initial scoping phase is documented in @appendix-aims, with the full project proposal provided in @appendix-proposal.
...
The three blocks are fused via late concatenation and evaluated across a dual-pipeline framework. This involves a classifier pipeline that predicts the *direction* of acquirer Cumulative Abnormal Return (CAR) at announcement, alongside a regression pipeline that attempts to predict CAR *magnitude*. As @ch-findings demonstrates, these two pipelines resolve differently - a divergence that is itself a substantive finding. To maintain strict leakage-control procedures, the pipeline enforces temporal splitting and an 11-day event-window embargo to reduce the risk of target leakage and market microstructure contamination.
...
*H1 - The Topological Alpha Hypothesis:* The inclusion of heterogeneous supply-chain graph topology is expected to yield a statistically significant improvement in directional deal discrimination (AUC-ROC) over a financial-only baseline, suggesting that topological data provides orthogonal predictive alpha.

*H2 - The Semantic Divergence Hypothesis:* The relationship between textual similarity and announcement returns appears conditional on document section. MD&A similarity (strategic fit) is expected to correlate positively with CAR, while Risk Factor similarity (shared liability exposure) is expected to correlate negatively. These opposing effects may be mathematically suppressed when the two sections are pooled.

*H3 - The Topological Arbitrage Hypothesis:* Acquirers with high betweenness centrality in the supply-chain graph are expected to exhibit statistically compressed variance in announcement return outcomes relative to peripheral acquirers, as measured by Levene's test across centrality quantile groups.

== What This Study Contributes <sec-intro-contributions>

The academic contribution of this work lies at the intersection of three fields that have not previously been unified in the context of post-merger synergy prediction. Supply chain finance, financial NLP, and heterogeneous graph neural networks have each advanced substantially as independent research programmes. To the best of current knowledge, this is the first study to direct their combination at binary CAR direction classification as a proxy for M&A synergy.

The contribution extends beyond the architectural. By reporting the M2 reversal—the finding that adding aggregated FinBERT text can actually *degrade* prediction quality—the study provides a methodological warning that NLP is not inherently useful in M&A prediction unless section semantics are preserved. Reporting negative $R^2$ values on the regression pipeline also helps define the boundaries of what multimodal architecture can achieve. Finally, the Deal Intelligence Terminal operationalises the theoretical framework into a live research artefact that allows for the interactive exploration of these empirical claims.

== Structure of the Dissertation <sec-intro-structure>

The dissertation proceeds as follows. @ch-litreview builds a sustained critical argument across four interconnected knowledge streams, demonstrating that each wave of prior scholarship produced an asymptotic ceiling and identifying precisely the structural information each paradigm discarded. @ch-methodology details the full research design, data pipeline, feature construction, and hypothesis-testing protocol. @ch-findings reports the empirical findings across both pipelines, including the negative results that establish the boundary conditions of the architecture. @ch-evaluation evaluates both the research product and the research process honestly, contextualising performance against the literature and identifying what should be done differently in future work. @ch-conclusion synthesises the findings and places the contribution in the context of the broader financial machine learning research agenda. Detailed implementation logic, ethical documentation, and terminal screenshots are provided in the Appendices (@appendix-aims to @appendix-code).
