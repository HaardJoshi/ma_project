...
The empirical record, however, tells a more troubling story. #cite(<martynova2008>, form: "prose") documented that between 70% and 90% of acquisitions fail to generate value for the acquirer's shareholders - a finding replicated across multiple independent research programmes @christensen2011. More striking than the magnitude of this failure rate is its persistence: it has remained statistically stable across entirely different economic eras, from the conglomerate wave of the 1960s through the technology boom of the late 1990s and the post-financial-crisis consolidation wave of the 2010s @martynova2008. A failure rate that survives such varied conditions is unlikely to be cyclical; it points toward something structurally wrong with how deals are evaluated.

#cite(<bradley1988>, form: "prose") established a revealing asymmetry in the data: while shareholders of acquired firms (the targets) captured average announcement-day abnormal returns of approximately 30%, the shareholders of acquiring firms systematically lost value, with combined deal synergy gains averaging only 7.4% of combined pre-deal firm value. The gap between what acquiring firms paid and what the market believed the combination was worth is precisely what this study attempts to predict.

=== Behavioural Distortions: When Executive Confidence Contaminates the Data

The first layer of explanation for persistent M&A failure is behavioural. #cite(<roll1986>, form: "prose") proposed the Hubris Hypothesis: acquiring managers systematically overestimate their own ability to extract value from a target, treating the market's price for that target as an underestimate rather than as a rational, information-weighted signal. This overconfidence drives them to pay premiums that the actual post-merger performance cannot justify - what #cite(<sirower1997>, form: "prose") termed the "Synergy Trap."
...
The first generation of quantitative M&A models - most influentially #cite(<palepu1986>, form: "prose") and #cite(<barnes1990>, form: "prose") - used logistic regression and multiple discriminant analysis on financial ratios. These approaches assume that each financial variable has a monotonic, linear relationship with the outcome: more of a given ratio always either increases or decreases the probability of a successful deal. #cite(<barnes1990>, form: "prose") demonstrated empirically that this assumption breaks down for leverage. Moderate debt signals fiscal discipline and managerial confidence; very high debt signals financial fragility. The relationship is non-monotonic - it changes direction at some threshold - and linear models fundamentally cannot represent direction changes.

More revealing still, #cite(<palepu1986>, form: "prose")'s own out-of-sample validation found that acquisition targets could not be predicted reliably beyond chance levels. The information contained in financial ratios was simply insufficient to reconstruct synergy outcomes — poor out-of-sample performance is the natural consequence. The consistently low pseudo-R² values (below 0.10) documented across econometric studies @betton2008 reflect the information gap inherent to financial ratio feature spaces, not model miscalibration. This ceiling is the first empirical benchmark any successor model must demonstrably improve upon.

=== Why Machine Learning Could Not Fully Escape the Same Trap

The arrival of ensemble machine learning methods - random forests, gradient-boosted decision trees, and ultimately XGBoost - promised to resolve the non-linearity problem. #cite(<zhang2024>, form: "prose") and similar studies deployed these techniques on financial ratio vectors and did report accuracy improvements over logistic baselines. Those improvements are real, but they are bounded by four structural problems that persist regardless of how sophisticated the algorithm becomes.

*The Independence Ceiling:* Both random forests and XGBoost operate on rows of a feature table where each row represents one deal in complete isolation. The algorithm learns patterns within individual deals but has no mechanism to propagate information between related deals. If two acquiring firms share a common supplier, and that supplier is experiencing financial distress, both deals carry a latent risk that is invisible in their individual financial ratios. Adding more features cannot resolve this — models that process each deal in isolation have no architectural mechanism for propagating information between related deals, regardless of feature richness @hamilton2017.
...
*Look-Ahead Contamination:* Several prominent ML studies, including #cite(<zhang2024>, form: "prose"), construct financial features from annual filings without explicitly verifying that the fiscal year end-date precedes the deal announcement date. When a model trains on financial data that postdates the deal it is predicting, its apparent accuracy reflects information that would not have been available at the time of the decision - inflating reported performance beyond any economically realisable level.
...
Despite this, a substantial body of recent research has continued to extend the BoW approach: #cite(<demers2024>, form: "prose") construct frequency-based lexicons for human capital disclosures; #cite(<acheampong2025>, form: "prose") proxy financial constraints using lexicon-based indices; #cite(<garcia2020>, form: "prose") develop finance-specific dictionaries validated through market data. Each of these contributions improves the vocabulary but cannot address the underlying compositional blindness. #cite(<hoberg2016>, form: "prose") represents the most structurally sophisticated BoW application - their text-based network industries classification used product-description cosine similarity from 10-K filings to construct time-varying firm similarity networks, demonstrating that textual proximity predicts competitive dynamics and deal likelihood.
This study inherits that semantic proximity methodology directly, but redirects it from predicting whether a deal will happen to predicting whether a deal that has happened will create or destroy value, and replaces TF-IDF word vectors with contextualised FinBERT embeddings.

=== The Transformer Revolution: A Better Tool, But Often Misdirected

#cite(<devlin2018>, form: "prose")'s BERT architecture marked a genuine qualitative advance in language modelling. Rather than counting word occurrences, BERT learns contextual embeddings - vector representations in which the same word receives a different numerical encoding depending on the surrounding words. This directly resolves the compositional blindness of BoW models. #cite(<araci2019>, form: "prose") specialized this architecture for financial language through pre-training on a large corpus of corporate filings, giving it domain-calibrated representations that a general BERT model lacks.

The challenge is not with the tools themselves but with how the field has applied them to M&A prediction. #cite(<zhao2020>, form: "prose") and #cite(<lohmeier2023>, form: "prose") both used transformer architectures to extract semantic features from M&A-related text, but then compressed the resulting rich, high-dimensional representations into a single scalar sentiment score before feeding them into a downstream classifier. This pipeline - transformer as sophisticated word counter, followed by scalar compression - preserves almost none of the representational richness that motivated using a transformer in the first place. The architecture is, in terms of information content, not substantially different from an advanced bag-of-words approach.

More fundamentally, #cite(<hajek2024>, form: "prose") and #cite(<lohmeier2023>, form: "prose") both direct their NLP pipelines at predicting *deal occurrence* - whether an acquisition announcement will happen - rather than *deal outcome* - whether the announced deal will create or destroy shareholder value.
These are genuinely different problems. Predicting deal occurrence requires detecting patterns in news flow that precede announcements; predicting deal outcome requires modelling the fundamental complementarity between acquirer and target across multiple dimensions. The existing NLP literature has made substantial progress on the former and has largely not addressed the latter.
...
#cite(<shi2017>, form: "prose") formalised the theoretical basis for Heterogeneous Information Networks (HINs) and demonstrated that type-specific attention mechanisms during neighbourhood aggregation consistently outperform homogeneous baselines on multi-relational graphs. #cite(<wang2021>, form: "prose") extended this into the Heterogeneous Graph Attention Network (HAN), showing that relationship-type-specific aggregation recovers semantic structure that single-type models discard. #cite(<lv2021>, form: "prose") provided systematic benchmarks confirming heterogeneous graph encoders achieve statistically superior performance across multiple classification tasks on multi-relational corporate networks.

While theoretical heterogeneous graph frameworks $cal(G) = (cal(V), cal(E), cal(T)_v, cal(T)_e)$ can encode four distinct edge relationships (`supplier_of`, `customer_of`, `competitor_of`, and `acquires`), this study explicitly restricts the implemented architecture to only two active supply-chain dependencies (`supplies` and `buys_from`). As detailed in @ch-methodology, this restriction is structurally necessary to prevent target leakage from competitor or acquisition edges. Type-specific GraphSAGE aggregation functions are applied independently per relationship type before cross-type pooling - recovering exactly the semantic distinctions that scalar feature engineering and homogeneous graph models discard.
...
The methodology operationalises this through a market model. For each acquirer $i$, a "normal" return generating process is estimated over a pre-announcement estimation window (@eq-lit-market-model):

$ R_(i,t) = alpha_i + beta_i R_(m,t) + epsilon_(i,t) $ <eq-lit-market-model>

where $R_(i,t)$ is the firm's daily return, $R_(m,t)$ is the broad market return, and $alpha_i$ and $beta_i$ are firm-specific parameters. Abnormal Returns (ARs) during the announcement period are then computed as the difference between actual and predicted returns (@eq-lit-ar):

$ A R_(i,t) = R_(i,t) - (hat(alpha)_i + hat(beta)_i R_(m,t)) $ <eq-lit-ar>
The Cumulative Abnormal Return ("CAR") sums these daily excess returns across the event window (@eq-lit-car):

$ "CAR"_i = sum_(t=t_1)^(t_2) A R_(i,t) $ <eq-lit-car>
...
#figure(
  table(
    ...
    // STREAM I: THE M&A PARADOX
    table.cell(colspan: 4, fill: luma(240))[*Stream I: The M&A Paradox & Valuation*],
    [Traditional Corporate Finance @roll1986 @sirower1997], [DCF / Valuation Models], [Behavioural contamination; executive hubris; market-timing noise], [Cumulative Abnormal Return (CAR) as objective market target],

    // STREAM II: TABULAR PARADIGM
    table.cell(colspan: 4, fill: luma(240))[*Stream II: The Tabular Paradigm*],
    [@palepu1986; @barnes1990], [Logit / MDA on ratios], [Linearity bias; pseudo-R² < 0.10; no network features], [Non-linear fusion engine; Blocks A+B+C],
    [@zhang2024], [XGBoost / Random Forest], [i.i.d. assumption; survivorship bias; look-ahead contamination], [GraphSAGE neighbourhood aggregation; temporal splits],
    [@elhoseny2022], [Deep MLP (AWOA-DL)], [Topologically flat; predicts financial distress, not synergy], [Relational graph operator; synergy CAR as target],

    // STREAM III: SEMANTIC TURN
    table.cell(colspan: 4, fill: luma(240))[*Stream III: The Semantic Turn*],
    [@loughran2011], [Bag-of-Words (BoW)], [Compositional blindness; deal occurrence target], [Contextual FinBERT embeddings; section-specific similarity (H2)],
    [@hajek2024], [FinBERT sentiment], [Single-document; no pairwise delta; acquisition likelihood target], [Pairwise acquirer-target cosine distance; H2 conditional directionality],
    [@zhao2020; @lohmeier2023], [BERT / RoBERTa → XGBoost], [Conflates strategic and risk disclosures; predicts occurrence not outcome], [Section-specific semantic splitting; binary CAR classification (H2)],
  ),
...
#figure(
  table(
    ...
    [@cohen2008; @ahern2014], [Industry-level supply chain analysis], [Industry-level only; no firm-level GNN; no CAR prediction], [Firm-level GraphSAGE on SPLC data (H1, H3)],
    [@venuti2021], [Homogeneous GraphSAGE], [Collapses edge semantics (suppliers = competitors); deal likelihood target], [Heterogeneous edge types; full multimodal fusion],
    [@baltrusaitis2019; @xu2021], [Multimodal fusion frameworks], [Not applied to M&A synergy; no graph modality], [Block A+B+C late fusion with SHAP decomposition],
  ),
