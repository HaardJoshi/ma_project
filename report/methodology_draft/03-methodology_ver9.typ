...
- *H1 (Topological Alpha):* Compare M5 (#code-inline("financial_graph.yaml"))
  vs. M3 (#code-inline("financial_only.yaml")) on the *Classifier Pipeline*.
  A paired $t$-test across cross-validation folds on held-out AUC-ROC scores
  assesses whether the graph stream yields a statistically significant
...
- *H2 (Semantic Divergence):* H2 is confirmed if and only if *both* conditions
  ...
  $t$-test across cross-validation folds (confirmatory ablation).  Neither
