...
the text stream adds semantic signal for the same; H3 tests whether all three
streams fused together outperform every individual modality in both regression
(magnitude) and classification (direction).
...
#figure(
  table(
    ...
    [The full tri-modal fusion (Financial + Text + Graph) significantly
     ...
     Success criterion: (a) Regressor Head --- statistically significant MAE
     and $R^2$ improvement, $p < 0.05$, Cohen's $d > 0.2$; (b) Classifier
     Head --- AUC-ROC improvement as secondary confirmatory evidence.
...
the architecture implements a *Dual-Evaluation Pipeline*.  The concatenated
vector $bold(z)_i$ is passed to two distinct prediction heads:

- *Regressor Head (H3):* A two-layer MLP with a *linear* output activation,
  optimised to predict continuous CAR via MSE loss.  This tests structural
  variance explanations.

- *Classifier Head (H1, H2):* A two-layer MLP with a *sigmoid* output
  activation, optimised to predict the binary direction of CAR ($1$ if
  $"CAR" > 0$, $0$ otherwise) via Binary Cross-Entropy loss.  This tests
  practical deal advisory discrimination.

Both heads share the same upstream concatenated representation $bold(z)_i$,
meaning the feature streams are trained once and evaluated on both objectives.
    ...
    [
      ...
      #grid(
        ...
        box(stroke: 0.4pt, inset: 6pt, radius: 3pt, width: 100%, fill: luma(245))[
          #align(center)[*Regressor Head (H3)*\ MLP + Linear activation\ $hat(y)_i in RR$ (CAR magnitude)\ Loss: MSE]
        ],
        align(center + horizon)[],
        box(stroke: 0.4pt, inset: 6pt, radius: 3pt, width: 100%, fill: luma(245))[
          #align(center)[*Classifier Head (H1, H2)*\ MLP + Sigmoid activation\ $hat(p)_i in [0,1]$ (CAR direction)\ Loss: BCE]
        ],
      )
...
- *Optimiser:* AdamW with cosine annealing learning-rate schedule with warm
  restarts.
- *Loss:* Two objectives trained simultaneously: MSE for the Regressor Head
  (continuous CAR); Binary Cross-Entropy for the Classifier Head (binary CAR
...
For variance analysis, the Regressor Head minimises Mean Squared Error against
...
For directional discrimination, the Classifier Head minimises Binary
...
#figure(
  ...
  caption: [Evaluation metrics. Regression metrics (MAE, RMSE, $R^2$, Huber) apply to the Regressor Head; classification metrics (Dir. Accuracy, AUC-ROC, F1) apply to the Classifier Head.],
) <tbl-metrics>
...
- *H1 (Topological Alpha):* Compare M5 (#code-inline("financial_graph.yaml"))
  vs. M3 (#code-inline("financial_only.yaml")) on the *Classification Pipeline*.
  A paired Diebold--Mariano test on hold-out AUC-ROC scores assesses whether
  the graph stream yields a statistically significant improvement in directional
  discrimination ($p < 0.05$).

- *H2 (Semantic Divergence):* H2 is confirmed if and only if *both* conditions
  hold: (a) a statistically significant Pearson/Spearman correlation between
  #code-inline("SemanticDiv_i") and $"CAR"_i$ ($p < 0.05$, primary test); and
  (b) M4 (#code-inline("financial_text.yaml")) yields a statistically significant
  AUC-ROC improvement over M3 (#code-inline("financial_only.yaml")) by paired
  DM test (confirmatory ablation).  Neither condition alone is sufficient.
...
@tbl-metrics.  The *Regressor Head* (H3) is evaluated on continuous CAR
...
The *Classifier Head* (H1, H2) is evaluated on binary CAR-direction
