...
=== Defence of the Market Model

The expected return model is also a potential point of attack. A single-factor market model is simpler than a Fama-French three-factor or five-factor alternative, and a reviewer may object that a broad market index is an imperfect beta proxy for smaller acquirers @fama1991. That criticism is theoretically legitimate. Nonetheless, the market model remains the implemented specification for three reasons.
