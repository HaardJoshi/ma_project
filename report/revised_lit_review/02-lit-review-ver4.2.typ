...
=== A Brief Primer: What "Semantic" Models Do

Where tabular models read financial numbers, semantic models read text. In the M&A context, this primarily means reading the annual 10-K reports that US-listed firms are required to file with the Securities and Exchange Commission. These filings contain structured, standardised sections: the Management Discussion and Analysis (MD&A), in which management describes the company's strategy, performance, and outlook in their own words, and the Risk Factors section, in which the company enumerates the specific threats and uncertainties it faces. The intuition motivating semantic M&A research is that these disclosures might reveal complementarity or conflict between acquirer and target that financial ratios alone cannot capture.
...
=== A Brief Primer: What "Topological" Models Do

Before examining the evidence, it is worth clarifying what is meant by "network topology" in this context and why it might matter for M&A prediction. Modern corporations do not exist in isolation — they are embedded in dense webs of economic relationships. A manufacturer buys components from dozens of suppliers. A retailer sells through hundreds of partners. Competitors share customers, regulatory exposure, and talent pools. These relationships create dependencies: if a key supplier fails, the customer's production line can stop. If a competitor is acquired and strengthens their market position, the remaining firm's pricing power erodes.

Topology refers to the *structure* of these relationship networks — not just which firms are connected, but how centrally positioned each firm is, how many paths between other firms run through it, and whether it serves as a critical bridge between otherwise disconnected clusters. The hypothesis motivating Stream IV is that this structural position encodes information about how a deal will be received by the market — information that balance sheets and even management narratives cannot convey.
...
Before describing the multimodal architecture, it is worth establishing precisely how this study measures "deal success" — a non-trivial problem given that the true synergies from a merger may take years to materialise.

The standard solution in empirical corporate finance, formalised by @mackinlay1997 and whose statistical properties were rigorously characterised by @brownwarner1985, is the event study. The core idea is elegant: if financial markets are reasonably efficient at incorporating publicly available information, then the stock price reaction to a deal announcement should reflect the market's best estimate of the present value of future synergies. A large positive price reaction means the market believes the deal will create substantial value; a negative reaction means the market believes it will destroy value.
...
#figure(
  table(
    ...
    table.header(
      [*Study / Stream*], [*Method*], [*Structural Limitation*], [*This Study's Response*],
    ),
    [Palepu (1986); Barnes (1990)], [Logit / MDA on ratios], [Linearity bias; pseudo-R² < 0.10; no network features], [Blocks A+B+C; non-linear fusion; graph topology],
    [Zhang et al. (2024)], [Random forest, XGBoost], [Independence assumption; survivorship bias; look-ahead contamination], [Temporal train/val/test splits; GraphSAGE neighbourhood aggregation],
    [Elhoseny et al. (2022)], [Deep MLP (AWOA-DL)], [Distress classification, not synergy; independence assumption unchanged], [Relational graph operator; synergy CAR as target],
    [Zhao et al. (2020); Han et al. (2023)], [BERT / RoBERTa → XGBoost], [Embeddings flattened to scalars; predicts deal occurrence not outcome], [FinBERT on section-split 10-Ks; binary CAR classification (H2)],
    ...
    [Loughran & McDonald (2011); Hoberg & Phillips (2016)], [BoW / TF-IDF similarity], [Compositional blindness; deal occurrence target], [Contextual FinBERT embeddings; section-specific similarity (H2)],
    ...
    [Venuti (2021)], [Homogeneous GraphSAGE], [Deal likelihood target; homogeneous edge types; no text or financial fusion], [Heterogeneous edge types; full multimodal fusion],
    [Baltrusaitis et al. (2019); Xu et al. (2021)], [Multimodal fusion frameworks], [Not applied to M&A synergy; no graph modality], [Full Block A+B+C late fusion with SHAP decomposition],
  ),
  caption: [Synthesis of four literature streams: methods, structural limitations, and architectural responses.],
) <tbl-gap>
...
*H2 — The Semantic Divergence Hypothesis:* The predictive relationship between textual similarity and synergy direction is conditional on document section. High cosine similarity in strategic disclosures (MD&A) will positively correlate with CAR — capturing strategic fit — whereas high similarity in Risk Factors will negatively correlate with CAR — capturing risk concentration. This conditional directionality is tested via bivariate OLS:
